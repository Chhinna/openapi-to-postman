 ...
 * @param {boolean} attribute - Whether the element is an attribute or not.
 * @param {string} indentChar - The character used for indentation.
 * @param {number} indent - The number of indentation levels.
 * @param {string} resolveTo - The type of value to resolve to, either 'example' or 'actual'.
 * @returns {string} The converted XML string.
 * @description
 * - If the schema is null or undefined, an empty string is returned.
 * - If the schema type is an integer, string, boolean, or number, the corresponding XML element is created with the resolved value.
 * - If the schema type is an object, the properties are converted to XML elements.
 * - If the schema type is an array, the items are converted to XML elements.
 * - If the schema type is an array and the 'wrapped' property is true, the items are wrapped in a parent element.
 */
